...
import { hash, HmacSha256, Message, Sha256 } from "../lib/sha256.ts";
import {
  assertEquals,
  assertObjectMatch,
...
const moduleDir = dirname(dirname(fromFileUrl(import.meta.url)));
...
Deno.test("[hash/sha256] test Uint8Array from Reader", async () => {
  ...
  const _hash = new Sha256().update(data, true);
  const hash = _hash.hex();
  const str = _hash.toString();
  _hash.update(data);
  assertEquals(
    hash,
...
Deno.test("[hash/sha256] hash function", () => {
  assertObjectMatch(
    hash("hello", "world") as unknown as Record<number, number>,
    Uint8Array.of(
      60,
      250,
      118,
  ...
  assertObjectMatch(
    hash("hello") as unknown as Record<number, number>,
    Uint8Array.of(
      44,
      242,
      77,
