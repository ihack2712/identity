...
// Adapted from deno to naturally support browsers without private fields.
...
export class Sha256 {
   ...
   */
  update(message: Message, __state__ = false): this {
    if (this._finalized) {
      return this;
    }
    ...
    if (this._bytes > 4294967295 || __state__) {
  ...
  arrayBuffer(): ArrayBuffer {
    ...
    const buffer = new ArrayBuffer(this._is224 ? 28 : 32);
    ...
    dataView.setUint32(20, this._h5);
    dataView.setUint32(24, this._h6);
    if (!this._is224) {
      dataView.setUint32(28, this._h7);
    }
    return buffer;
  }
}
...
export class HmacSha256 extends Sha256 {
  ...
  private _oKeyPad: number[];
  private _sharedMemory: boolean;
  ...
  protected finalize(): void {
    ...
    if (this._inner) {
      ...
      super.init(this._is224, this._sharedMemory);
      this.update(this._oKeyPad);
...
export function hash(message: Message, secret?: Message): Uint8Array {
  const hash = secret
    ? new HmacSha256(secret, false, false)
    : new Sha256(false, false);
  return new Uint8Array(hash.update(message).array());
}
